interface CssVarDesign {
  ...
  spacing: { scale: Array<number | string> };
  borders: { radii: Array<{ label: string; value: number }> };
  shadows: { values: Array<{ label: string; raw: string }> };
  variables: Record<string, Record<string, unknown>>;
}

export function formatCssVars(design: CssVarDesign): string {
  const lines = [":root {"];

  // Colors
  lines.push("  /* Colors — Primary */");
  if (design.colors.primary)
    lines.push(`  --color-primary: ${design.colors.primary.hex};`);
  if (design.colors.secondary)
    lines.push(`  --color-secondary: ${design.colors.secondary.hex};`);
  if (design.colors.accent)
    lines.push(`  --color-accent: ${design.colors.accent.hex};`);
  lines.push("");
  ...
  if (design.colors.neutrals.length > 0) {
    ...
    for (let i = 0; i < design.colors.neutrals.length && i < 10; i++) {
      lines.push(
        `  --color-neutral-${i * 100 || 50}: ${design.colors.neutrals[i].hex};`,
      );
    }
    lines.push("");
  }
  ...
  if (design.colors.backgrounds.length > 0) {
    ...
    for (let i = 0; i < design.colors.backgrounds.length; i++) {
      lines.push(
        `  --color-bg${i > 0 ? `-${i}` : ""}: ${design.colors.backgrounds[i]};`,
      );
    }
    lines.push("");
  }
  ...
  if (design.colors.text.length > 0) {
    ...
    for (let i = 0; i < design.colors.text.length && i < 5; i++) {
      lines.push(
        `  --color-text${i > 0 ? `-${i}` : ""}: ${design.colors.text[i]};`,
      );
    }
    lines.push("");
  }
  ...
  const siteVars = Object.entries(design.variables).filter(([, v]) =>
