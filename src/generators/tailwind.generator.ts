...
export interface TailwindAssistantPack {
  codex: string;
  cursor: string;
  claude: string;
  copilot: string;
...
class TailwindThemeGenerator {
  ...
  private generateTypography(design: DesignSystem): void {
    ...
    for (const s of design.typography.scale.slice(0, 15)) {
      ...
      const fontOptions: Record<string, string | number> = {};
      if (lineHeight !== undefined) fontOptions.lineHeight = lineHeight;
      if (letterSpacing !== undefined) fontOptions.letterSpacing = letterSpacing;

      this.config.fontSize![`${s.size}`] = [
        `${s.size}px`,
        Object.keys(fontOptions).length > 0 ? fontOptions : undefined,
      ];
    }
  }
  ...
  private generateBreakpoints(design: DesignSystem): void {
    for (const bp of design.breakpoints) {
      if (bp.type === "min-width") {
        this.config.screens![bp.label] = this.normalizeSize(bp.value);
      }
    }
  }
...
export function buildTailwindAssistantPack(design: DesignSystem): TailwindAssistantPack {
  const config = formatTailwind(design);
  const sections = [
    "Use this Tailwind config exactly as generated by FlyRank Visual DNA.",
    "",
    "```ts",
    config.trim(),
    "```",
    "",
    "Rules:",
    "- Keep token names unchanged.",
    "- Reuse spacing, radius, and color values from the config only.",
    "- Do not invent additional palette shades unless explicitly requested.",
  ].join("\n");

  return {
    codex: `Implement UI using this Tailwind config.\n\n${sections}`,
    cursor: `# Tailwind Integration\n\n${sections}`,
    claude: `Apply this Tailwind theme in your artifact output.\n\n${sections}`,
    copilot: `Scaffold components against this Tailwind config.\n\n${sections}`,
    generic: sections,
  };
}
