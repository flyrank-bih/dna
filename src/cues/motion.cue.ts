...
interface MotionKeyframe {
  name: string;
  steps: MotionKeyframeStep[];
}

interface MotionExtractionResult {
  durations: Array<{ name: string; ms: number; css: string }>;
  easings: Array<{ family: string; raw: string; count: number; overshoot?: boolean }>;
  springs: Array<{ family: string; raw: string; count: number; overshoot?: boolean }>;
  keyframes: Array<{
    name: string;
    steps: MotionKeyframeStep[];
    kind: string;
  ...
  transitionedProperties: Array<{ property: string; count: number }>;
  scrollLinked: { present: boolean; signals: string[] };
  stats: {
    animatingElements: number;
    transitionCount: number;
    keyframeCount: number;
    keyframeUnused: number;
  };
  feel: string;
}
...
function nameDuration(ms: number) {
  return DURATION_NAMES.find((d) => ms <= d.max)?.name || "xxl";
}

function classifyCubicBezier(raw: string) {
  const m = raw.match(
  ...
  if (y1 < 0 || y2 > 1 || y2 < 0 || y1 > 1)
    return { family: "spring", raw, overshoot: true };
...
function classifyEasing(raw: string) {
  if (!raw) return { family: "linear", raw };
  if (raw === "linear") return { family: "linear", raw };
  if (raw === "ease" || raw === "ease-in-out")
    return { family: "ease-in-out", raw };
...
function isBounceKeyframe(kf: MotionKeyframe) {
...
function keyframeKind(kf: MotionKeyframe) {
  ...
  const has = (p: string) => props.has(p);
  const anyValue = (re: RegExp) => values.some((v) => re.test(v));
...
export class MotionCueExtractor
  implements
    CueExtractor<
      [computedStyles: MotionComputedStyle[], keyframes?: MotionKeyframe[]],
      MotionExtractionResult
    >
{
  extract(
    computedStyles: MotionComputedStyle[] = [],
    keyframes: MotionKeyframe[] = [],
  ): MotionExtractionResult {
  const transitions = new Set<string>();
  const easingRaw = new Set<string>();
  const durations: number[] = [];
  const animationRefs = new Map<string, number>();
  const transitionedProps: Record<string, number> = {};
  const scrollSignals = new Set<string>();
  ...
  for (const el of computedStyles) {
    ...
    if (
      ...
      for (const m of transition.matchAll(
        /(?<![(\d])(\d{1,8}(?:\.\d{1,4})?m?s)(?![)\w])/g,
      ))
        durations.push(MS(m[1]));
      // Tightened: limit cubic-bezier/steps inner content to 64 chars.
      for (const m of transition.matchAll(
        /(ease|ease-in|ease-out|ease-in-out|linear|cubic-bezier\([^)]{1,64}\)|steps\([^)]{1,64}\))/g,
      ))
        easingRaw.add(m[1]);
      for (const part of transition.split(",")) {
        const prop = part.trim().split(/\s+/)[0];
        if (prop && prop !== "all")
          transitionedProps[prop] = (transitionedProps[prop] || 0) + 1;
      }
    }
    ...
    if (
      ...
      const nameMatch =
        animation.match(/([a-zA-Z_][\w-]{0,127})$/) ||
        animation.match(/^([a-zA-Z_][\w-]{0,127})/);
      if (nameMatch) {
    ...
    if (
      ...
      el.animationTimeline !== "auto" &&
      el.animationTimeline !== "none" &&
      el.animationTimeline !== ""
    ) {
      scrollSignals.add(el.animationTimeline);
    }
    if (el.viewTimelineName) scrollSignals.add(`view:${el.viewTimelineName}`);
    if (el.scrollTimelineName)
      scrollSignals.add(`scroll:${el.scrollTimelineName}`);
    if (isAnimating) animatingElements++;
  }

  const uniqueDurations = [...new Set(durations.filter((d) => d > 0))].sort(
    (a, b) => a - b,
  );
  ...
  const namedDurations: Array<{ name: string; ms: number; css: string }> = [];
  ...
  const easings = [...easingRaw].map((e) => ({
    ...classifyEasing(e),
    count: computedStyles.filter((c) => (c.transition || "").includes(e))
      .length,
  }));
  ...
  const transitionTop = Object.entries(transitionedProps)
    .sort((a, b) => b[1] - a[1])
    .slice(0, 10)
  ...
  const share = (family: string) =>
    easings
      .filter((e) => e.family === family)
      .reduce((s, e) => s + e.count, 0) / (totalUses || 1);
  const feel =
    springs.length > 0
      ? "springy"
      : share("ease-out") > 0.5
...
export function extractMotion(
  ...
  return new MotionCueExtractor().extract(computedStyles, keyframes);
}
