...
interface LibraryContext {
  ...
  scripts: string;
  tailwindLike: number;
  radixAttrCount: number;
}

interface LibraryFingerprint {
  id: string;
  score: (ctx: LibraryContext) => { score: number; evidence: string[] };
}

const LIB_FINGERPRINTS: LibraryFingerprint[] = [
  {
    id: "shadcn/ui",
    score: (ctx: LibraryContext) => {
      let s = 0;
      const evidence = [];
      // shadcn copies Radix attributes and uses Tailwind utility density.
      if (ctx.radixAttrCount > 2 && ctx.tailwindLike > 0.4) {
        s += 0.5;
        evidence.push("radix+tailwind mix");
      }
      if (
        /\bbutton-primary|\bbutton-destructive/.test(ctx.classBlob) &&
        ctx.radixAttrCount > 0
      ) {
        s += 0.15;
        evidence.push("shadcn button tokens");
      }
      // `class="... bg-background text-foreground ..."` is a shadcn tell.
      if (
        /\bbg-background\b|\btext-foreground\b|\bborder-input\b|\bring-offset-background\b/.test(
          ctx.classBlob,
        )
      ) {
        s += 0.65;
        evidence.push("shadcn css tokens");
      }
  ...
  },
  {
    id: "headlessui",
    score: (ctx: LibraryContext) => {
      const m = (ctx.classSample.join(" ").match(/headlessui-[a-z]+/gi) || [])
        .length;
      if (m < 2) return { score: 0, evidence: [] };
      return {
        score: Math.min(0.9, 0.3 + m * 0.08),
        evidence: [`${m} headlessui- class refs`],
      };
    },
  },
  {
    ...
    score: (ctx: LibraryContext) => {
      ...
      if (m < 2) return { score: 0, evidence: [] };
      return {
        score: Math.min(0.95, 0.4 + m * 0.04),
        evidence: [`${m} Mui*-root classes`],
      };
    },
  },
  {
    ...
    score: (ctx: LibraryContext) => {
      ...
      return {
        score: Math.min(0.95, 0.4 + m * 0.03),
        evidence: [`${m} chakra- classes`],
      };
    },
  },
  {
  ...
  },
  {
    ...
    score: (ctx: LibraryContext) => {
      ...
      return {
        score: Math.min(0.9, 0.3 + hits * 0.1),
        evidence: [`bootstrap utility hits: ${hits}`],
      };
    },
  },
  {
    ...
    score: (ctx: LibraryContext) => {
      ...
      if (m < 2) return { score: 0, evidence: [] };
      return {
        score: Math.min(0.95, 0.4 + m * 0.05),
        evidence: [`${m} heroui/nextui classes`],
      };
    },
  },
  {
    ...
    score: (ctx: LibraryContext) => {
      ...
      return {
        score: 0.3 + hits * 0.12,
        evidence: [
          `tailwind density=${ctx.tailwindLike.toFixed(2)}, pattern hits=${hits}`,
        ],
      };
    },
  },
  {
    ...
    score: (ctx: LibraryContext) => {
      ...
      return {
        score: Math.min(0.9, 0.3 + m * 0.02),
        evidence: [`${m} v-* classes`],
      };
    },
  },
  {
    ...
    score: (ctx: LibraryContext) => {
      ...
      return {
        score: 0.3 + (ctx.tailwindLike - 0.35) * 1.2,
        evidence: [
          `tailwind-like class density ${(ctx.tailwindLike * 100).toFixed(0)}%`,
        ],
      };
    },
  },
...
function computeTailwindLike(classSample: string[]): number {
...
export class ComponentLibraryExtractor {
  extract(stackSignals: StackSignals = {}) {
  ...
  const ranked: Array<{ id: string; score: number; evidence: string[] }> = [];
  ...
  // the real answer.
  const hasHigherLevel = ranked.some(
    (r) => r.id !== "tailwindcss" && r.id !== "tailwind-ui" && r.score > 0.35,
  );
  if (hasHigherLevel) {
  ...
  const primary = ranked[0] || { id: "unknown", score: 0, evidence: [] as string[] };
...
export function extractComponentLibrary(stackSignals: StackSignals = {}) {
  return new ComponentLibraryExtractor().extract(stackSignals);
}
