...
function rgbSaturation(rgb: { r: number; g: number; b: number } | null): number {
  if (!rgb) return 0;
...
function avgSaturation(colors: Array<{ hex?: string } | string>): number {
...
function detectBackdropBlur(
  modernCss: { pseudoElements?: { samples?: string[] } } = {},
  ...
  const samples = [
    ...
    ...Object.values(variables || {}).flatMap((v) =>
      typeof v === "string" ? [v] : [],
    ),
  ].join(" ");
...
function shadowComplexity(shadowValues: Array<string | { value?: string }>) {
  ...
  for (const v of shadowValues) {
    ...
    const blurs = [
      ...raw.matchAll(
        /(-?\d{1,8}(?:\.\d{1,4})?)(?:px)?\s+(-?\d{1,8}(?:\.\d{1,4})?)(?:px)?\s+(\d{1,8}(?:\.\d{1,4})?)(?:px)?/g,
      ),
    ];
...
function borderProfile(radii: Array<number | string | { value?: string; px?: number }> = []) {
  const nums = radii
    .map((r) => {
      if (typeof r === "number") return r;
      if (typeof r === "string") return parseFloat(r) || 0;
      if (r && typeof r === "object")
        return Number(r.value ?? r.px ?? 0) || 0;
      return 0;
    })
    .filter((n) => !isNaN(n));
...
interface MaterialDesignInput {
  colors?: { all?: Array<{ hex?: string } | string> };
  shadows?: { values?: Array<string | { value?: string }> };
  borders?: { radii?: Array<number | string | { value?: string; px?: number }> };
  variables?: Record<string, unknown>;
  modernCss?: { pseudoElements?: { samples?: string[] } };
  gradients?: { count?: number };
}

interface MaterialLanguageResult {
  label: string;
  confidence: number;
...
export class MaterialLanguageCueExtractor
  implements CueExtractor<[design: MaterialDesignInput], MaterialLanguageResult>
{
  extract(design: MaterialDesignInput = {}): MaterialLanguageResult {
    ...
    const scores = Object.fromEntries(LABELS.map((l) => [l, 0])) as Record<
      string,
      number
    >;
    const signals: Array<{ label: string; weight: number; detail: string }> = [];

  if (hasBackdropBlur) {
    scores.glassmorphism += 0.6;
    signals.push({
      label: "glassmorphism",
      weight: 0.6,
      detail: "backdrop-filter present",
    });
  }
  if (sh.avgBlur > 30 && sat < 0.3 && sh.insetCount > 0 && sh.hasPair) {
    scores.neumorphism += 0.7;
    signals.push({
      label: "neumorphism",
      weight: 0.7,
      detail: "paired blur + inset + low saturation",
    });
  }
  if (sh.profile === "hard" && br.sharp && sat > 0.4) {
    scores.brutalist += 0.75;
    signals.push({
      label: "brutalist",
      weight: 0.75,
      detail: "hard shadows + sharp corners + saturated",
    });
  }
  if (
    shadows.length === 0 &&
    ...
    signals.push({
      label: "flat",
      weight: 0.55,
      detail: "no shadows, simple radii",
    });
  }
  if (sh.avgBlur > 60 && sat < 0.4 && !hasBackdropBlur) {
    ...
    signals.push({
      label: "soft-ui",
      weight: 0.5,
      detail: "soft diffuse shadows",
    });
  }
  if (br.pill && sh.profile === "soft" && sat > 0.3) {
    scores["material-you"] += 0.45;
    signals.push({
      label: "material-you",
      weight: 0.45,
      detail: "pill shapes + soft shadows",
    });
  }
  if (gradientCount > 6 && sat > 0.5) {
    scores.skeuomorphic += 0.35;
    signals.push({
      label: "skeuomorphic",
      weight: 0.35,
      detail: "heavy gradient usage",
    });
  }

    const ranked = Object.entries(scores).sort((a, b) => b[1] - a[1]);
    const [winner = "flat", winScore = 0] = ranked[0] || [];
    ...
    return {
      ...
      alternates: ranked
        ...
        .slice(0, 3)
        .map(([l, s]) => ({ label: l, score: Number(s.toFixed(3)) })),
    };
  }
}

export function extractMaterialLanguage(
  design: MaterialDesignInput = {},
): MaterialLanguageResult {
  return new MaterialLanguageCueExtractor().extract(design);
}
