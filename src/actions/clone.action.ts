...
function dedupeConsecutive(order: string[]): string[] {
  const out: string[] = [];
  for (const r of order) if (r !== out[out.length - 1]) out.push(r);
  return out;
}

function sanitize(str: unknown, fallback = ""): string {
...
function pickHeading(
  voice: CloneDesignInput["voice"] | undefined,
  fallback: string,
): string {
  const s = (voice?.sampleHeadings || []).find(
    (h) => h && h.length > 4 && h.length < 80,
  );
  return s || fallback;
}

function materialPreset(material: { label?: string } = {}) {
  ...
  switch (label) {
    case "brutalist":
      return {
        ...
        cardBorder: "2px solid var(--color-foreground)",
      };
    case "glass":
      return {
        radius: "16px",
        ...
        cardBorder: "1px solid var(--color-neutral-2)",
    ...
    case "neumorphism":
      return {
        radius: "24px",
        shadow:
          "12px 12px 24px rgba(0,0,0,0.08), -12px -12px 24px rgba(255,255,255,0.6)",
        border: "none",
        cardBorder: "none",
      };
    case "flat":
...
function primaryCta(voice: CloneDesignInput["voice"] | undefined): string {
  const verb = voice?.ctaVerbs?.[0]?.value || "Get started";
  return verb.charAt(0).toUpperCase() + verb.slice(1);
}

function secondaryCta(voice: CloneDesignInput["voice"] | undefined): string {
...
interface RenderContext {
  ...
  intent: string;
  url: string;
  mat: ReturnType<typeof materialPreset>;
...
function renderHero(ctx: RenderContext): string {
  ...
  const lede = pickHeading(
    voice,
    `A ${intent || "product"} that deserves its own design system.`,
  );
...
function renderFeatureGrid(ctx: RenderContext): string {
  ...
  const features = [
    {
      t: "Primitives",
      b: "Every color, type, shadow and spacing value lifted from the source.",
    },
    {
      t: "Anatomy",
      b: "Variants, sizes and states inferred from real DOM — not guessed.",
    },
    {
      t: "Voice",
      b: "The verbs your CTAs use, the rhythm of your headings, preserved.",
    },
    {
      t: "Drift",
      b: "Audit your tokens against production on every PR. Exit non-zero on divergence.",
    },
    {
      t: "Platforms",
      b: "One extraction → iOS, Android, Flutter, WordPress, MCP.",
    },
    {
      t: "Prompts",
      b: "Paste into v0 or Lovable and get the design right on the first try.",
    },
  ];
  return `
...
function renderFaq() {
  const qas = [
    [
      ...
      "Every token and the section reading order come from the real DOM. Copy is filler — the design is real.",
    ],
    [
      "Does it use an LLM?",
      "No. Everything runs from Playwright + deterministic classifiers. --smart is optional.",
    ],
    [
      "Can it handle auth?",
      "Yes — pass --cookie-file for Netscape cookies, Playwright storageState, or a JSON array.",
    ],
    [
      "Is this legal to use on competitors?",
      "It captures publicly rendered CSS/DOM. Use respectfully; the tokens belong to the source.",
    ],
  ];
  return `
      <section style={{ padding: '96px 0', maxWidth: '760px' }}>
...
function renderSteps() {
  const steps = [
    [
      "01",
      "Extract",
      "Point flydesign at any URL. Get DTCG tokens in seconds.",
    ],
    [
      "02",
      "Emit",
      "One extraction fans out to Tailwind, CSS vars, Figma vars, iOS, Android, Flutter.",
    ],
    [
      "03",
      "Ship",
      "Clone a starter, add the CI bot, or launch the Studio to share.",
    ],
  ];
  return `
      <section style={{ padding: '96px 0' }}>
        ...
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 14 }}>
          <thead>
            <tr>
              ...
              <th style={{ textAlign: 'left', padding: '12px 16px', borderBottom: '1px solid var(--color-foreground)', fontWeight: 600 }}>flydesign</th>
...
function renderFooter(url: string): string {
  return `
      <footer style={{ padding: '48px 0 32px', borderTop: '1px solid var(--color-neutral-2)', fontSize: 13, color: 'var(--color-neutral-1)', display: 'flex', justifyContent: 'space-between', flexWrap: 'wrap', gap: 16 }}>
        ...
        <div style={{ fontFamily: 'var(--font-mono)' }}>flydesign clone</div>
      </footer>`;
}
...
interface CloneDesignInput {
  ...
  voice?: {
    ...
    ctaVerbs?: Array<{ value?: string }>;
    sampleHeadings?: string[];
  };
  sectionRoles?: { readingOrder?: string[] };
  materialLanguage?: { label?: string; confidence?: number };
  pageIntent?: { type?: string; confidence?: number };
...
function generateCloneProject(design: CloneDesignInput, outDir: string): CloneResult {
  ...
  const monoFont =
    typography.families.find((f) => f.name.toLowerCase().includes("mono"))
      ?.name || "monospace";
  const radiusMd = borders.radii.find((r) => r.label === "md")?.value || 8;
  const shadowMd =
    shadows.values.find((s) => s.label === "md")?.raw ||
    "0 4px 6px rgba(0,0,0,0.1)";
  const neutrals = colors.neutrals.slice(0, 5);
  const metaTitle = design.meta?.title || "cloned";
...
`,
  ...
  // layout.js
  writeFileSync(
    join(projectDir, "src/app/layout.js"),
    `export const metadata = {
  title: '${metaTitle.replace(/\\/g, "\\\\").replace(/'/g, "\\'")} · cloned',
  description: 'Design cloned from ${url} with flydesign.',
};
...
`,
  ...
  const rawOrder =
    Array.isArray(sectionRoles.readingOrder) && sectionRoles.readingOrder.length
  ...
  const order = dedupeConsecutive(rawOrder as string[]);
...
`,
  ...
  writeFileSync(
    ...
    `# ${metaTitle || "Cloned design"}

Generated by package API clone function for ${url}.
...
- **Voice:** tone *${voice.tone || "neutral"}*, headings *${voice.headingStyle || "sentence"}* / *${voice.headingLengthClass || "balanced"}*, top CTAs: ${
      (voice.ctaVerbs || [])
        .slice(0, 3)
        .map((v) => v.value)
        .join(", ") || "—"
...
export class CloneAction
  implements ActionHandler<[design: CloneDesignInput, outDir: string], CloneResult>
{
  run(design: CloneDesignInput, outDir: string): CloneResult {
    return generateCloneProject(design, outDir);
  }
}

export function generateClone(
  design: CloneDesignInput,
  outDir: string,
): CloneResult {
  return new CloneAction().run(design, outDir);
}
