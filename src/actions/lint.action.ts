...
function flattenDtcg(obj: unknown, prefix = "", out: FlatToken[] = []): FlatToken[] {
  for (const [k, v] of Object.entries((obj || {}) as Record<string, unknown>)) {
    if (k.startsWith("$")) continue;
    if (v && typeof v === "object") {
      if ("$value" in (v as Record<string, unknown>)) {
        const token = v as { $value?: unknown; $type?: unknown };
        out.push({
          name: prefix ? `${prefix}.${k}` : k,
          value: String(token.$value ?? ""),
          type: typeof token.$type === "string" ? token.$type : undefined,
        });
      } else {
...
function loadTokens(file: string): FlatToken[] {
...
function dedupeClose(
  hexes: Array<{ name: string; value: string }>,
  threshold = 8,
): Array<{ a: string; b: string; distance: number }> {
  const near: Array<{ a: string; b: string; distance: number }> = [];
  for (let i = 0; i < hexes.length; i++) {
    for (let j = i + 1; j < hexes.length; j++) {
      const a = hexToRgb(hexes[i].value),
        b = hexToRgb(hexes[j].value);
      if (!a || !b) continue;
...
function checkScale(values: string[], label: string) {
  const nums = values
    .map(parsePx)
  ...
  const variance =
    ratios.reduce((s, r) => s + (r - avg) ** 2, 0) / (ratios.length || 1);
  return {
    label,
    count: nums.length,
    values: nums,
    avgRatio: +avg.toFixed(2),
    variance: +variance.toFixed(3),
  };
}

interface LintTokensResult {
  file: string;
  tokenCount: number;
  score: number;
  grade: "A" | "B" | "C" | "D" | "F";
  ...
  stats: { colors: number; spacing: number; radii: number; shadows: number };
}

export class LintTokensAction implements ActionHandler<[file: string], LintTokensResult> {
  run(file: string): LintTokensResult {
  ...
  const colors = tokens
    .filter(isColor)
    .map((t) => ({ name: t.name, value: toHex(t.value) || t.value }))
    .filter((t) => /^#[\da-f]{6}$/i.test(t.value));
  ...
  const spaceCheck = checkScale(
    spacing.map((t) => t.value),
    "spacing",
  );
  ...
  scorecard.spacingSystem = spaceCheck
    ? Math.max(30, 100 - Math.round(spaceCheck.variance * 200))
    : 50;
  ...
  scorecard.borderRadii = Math.max(
    30,
    100 - Math.max(0, radii.length - 6) * 10,
  );
  ...
  const fgs = palette.filter((c) =>
    /fg|text|foreground|ink|label/i.test(c.name),
  );
  const contrastFails: Array<{ fg: string; bg: string; ratio: number }> = [];
  for (const fg of fgs) {
    for (const bg of bgs) {
      const fgRgb = hexToRgb(fg.value),
        bgRgb = hexToRgb(bg.value);
      if (!fgRgb || !bgRgb) continue;
      const c = contrast(fgRgb, bgRgb);
      if (c < 4.5)
        contrastFails.push({ fg: fg.name, bg: bg.name, ratio: +c.toFixed(2) });
    }
  }
  ...
  scorecard.accessibility = contrastFails.length
    ? Math.max(20, 100 - contrastFails.length * 12)
    : 100;
...
export function lintTokens(file: string): LintTokensResult {
  return new LintTokensAction().run(file);
}
