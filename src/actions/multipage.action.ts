...
function rankCandidateByUrl(href: string): string[] {
  try {
    const u = new URL(href);
    const path = (u.pathname || "/").toLowerCase();
    const hits = [];
    if (/\/pricing(\/|$)|\/plans(\/|$)/.test(path)) hits.push("pricing");
    if (/\/docs?(\/|$)|\/documentation|\/guide|\/reference/.test(path))
      hits.push("docs");
    ...
    if (/\/product(\/|$)|\/features?(\/|$)|\/solutions?(\/|$)/.test(path))
      hits.push("product");
...
function scoreAnchor(anchorText: string): Record<string, number> {
...
interface CanonicalPageTarget {
  type: string;
  url: string;
  anchor: string;
}

interface CrawlCanonicalPagesOptions {
  homepageUrl: string;
  homepageRawData?: unknown;
  maxPages?: number;
  extract: (url: string, options?: Record<string, unknown>) => Promise<PageDesign>;
  crawlerOptions?: Record<string, unknown>;
}

interface PageDesign {
  colors?: { all?: Array<{ hex?: string }> };
  typography?: { families?: string[] };
  spacing?: { scale?: Array<{ value?: number } | number> };
  borders?: { radii?: Array<{ value?: number } | number> };
  raw?: Record<string, unknown>;
  meta?: { title?: string };
}

export async function discoverCanonicalPages(
  links: LinkCandidate[],
  homepageUrl: string,
  maxPages = 5,
): Promise<CanonicalPageTarget[]> {
  const base = new URL(homepageUrl);
  const byType: Record<string, Array<{ href: string; text: string }>> = {};
  for (const link of links) {
    let href;
    try {
      href = new URL(link.href, base).toString();
    } catch {
      continue;
    }
    try {
  ...
  const chosen: CanonicalPageTarget[] = [];
...
export async function collectLinks(page: Page): Promise<LinkCandidate[]> {
...
function pickChoices(aAll: string[] = [], bAll: string[] = []) {
...
function hexSet(colors?: PageDesign["colors"]): Set<string> {
  return new Set(
    (colors?.all || []).map((c) => (c.hex || "").toLowerCase()).filter(Boolean),
  );
}

function typeSet(typography?: PageDesign["typography"]): Set<string> {
  return new Set((typography?.families || []).map((f) => f.toLowerCase()));
}

function spaceSet(spacing?: PageDesign["spacing"]): Set<string> {
  return new Set(
    (spacing?.scale || []).map((s) =>
      String(typeof s === "number" ? s : (s?.value ?? "")),
    ),
  );
}

function radiusSet(borders?: PageDesign["borders"]): Set<string> {
  return new Set(
    (borders?.radii || []).map((r) =>
      String(typeof r === "number" ? r : (r?.value ?? "")),
    ),
  );
}

export function computeCrossPageConsistency(
  pages: Array<{ url: string; type?: string; design: PageDesign }>,
): {
  pairwise: Array<Record<string, unknown>>;
  drift: Record<string, unknown>;
  shared: { colors: string[]; typography: string[] };
  perPageUnique?: Array<{ url: string; type?: string; uniqueColors: string[] }>;
} {
  if (pages.length < 2) {
    return { pairwise: [], drift: {}, shared: { colors: [], typography: [] } };
  }
  ...
  for (let i = 0; i < pages.length; i++) {
    for (let j = i + 1; j < pages.length; j++) {
      const a = pages[i],
        b = pages[j];
      const colorDiff = pickChoices(
        [...hexSet(a.design.colors)],
        [...hexSet(b.design.colors)],
      );
      const typeDiff = pickChoices(
        [...typeSet(a.design.typography)],
        [...typeSet(b.design.typography)],
      );
      const spaceDiff = pickChoices(
        [...spaceSet(a.design.spacing)],
        [...spaceSet(b.design.spacing)],
      );
      const radiusDiff = pickChoices(
        [...radiusSet(a.design.borders)],
        [...radiusSet(b.design.borders)],
      );
      pairwise.push({
  ...
  const sharedColors = pages.reduce((acc: Set<string>, p, i) => {
    const set = hexSet(p.design.colors);
    if (i === 0) return set;
    return new Set([...acc].filter((x) => set.has(x)));
  }, new Set<string>());
  const sharedTypes = pages.reduce((acc: Set<string>, p, i) => {
  ...
  const perPageUnique = pages.map((p, idx) => {
    ...
    const othersColors = others.reduce((s: Set<string>, o) => {
      for (const x of hexSet(o.design.colors)) s.add(x);
      return s;
    }, new Set<string>());
    const unique = [...hexSet(p.design.colors)].filter(
      (c) => !othersColors.has(c),
    );
...
interface CanonicalPageRunResult {
  targets: CanonicalPageTarget[];
  pages: Array<{ url: string; type?: string; intent: unknown; error: string | null }>;
  consistency: ReturnType<typeof computeCrossPageConsistency>;
}

export async function crawlCanonicalPages({
  homepageUrl,
  homepageRawData: _homepageRawData,
  maxPages = 5,
  extract,
  crawlerOptions = {},
  ...
  const context = await browser.newContext({
    viewport: {
      ...
      height: Number(crawlerOptions.height) || 800,
    },
    colorScheme: "light",
  });
  ...
  try {
    ...
    for (const t of targets) {
      try {
        ...
        perPage.push({ url: t.url, type: t.type, error: message });
      }
    }
    // Include the homepage itself in the consistency pass if we have its design.
    const allPages: Array<{ url: string; type?: string; design: PageDesign }> =
      perPage.filter(
        (p): p is { url: string; type: string; design: PageDesign } =>
          "design" in p,
      );
    ...
    return {
      ...
      pages: perPage.map((p) => ({
        ...
        type: p.type,
        intent: "design" in p
          ? (extractPageIntent((p.design.raw || {}) as Parameters<typeof extractPageIntent>[0], {
              url: p.url,
              title: p.design.meta?.title,
            }) as unknown)
          : null,
...
export class CrawlCanonicalPagesAction
  implements ActionHandler<[opts: CrawlCanonicalPagesOptions], Promise<CanonicalPageRunResult>>
{
  async run(opts: CrawlCanonicalPagesOptions): Promise<CanonicalPageRunResult> {
    return crawlCanonicalPages(opts);
  }
}
