...
function toMp4(webmPath: string): string | null {
...
async function recordReplayRuntime(
  url: string,
  opts: ReplayOptions = {},
): Promise<ReplayResult> {
  const outDir = resolve(opts.out || "./.flydesign");
  mkdirSync(outDir, { recursive: true });
  const prefix = opts.prefix || "motion-replay";
  const duration = Math.max(2, Math.min(15, Number(opts.duration) || 5));
    ...
    if (i === Math.floor(steps / 2)) {
      try {
        const btn = await page.$(
          'button, [role="button"], a.btn, .button, [class*="btn"]',
        );
  ...
  try {
    ...
    if (video && typeof video.path === "function") {
      ...
      const produced = readdirSync(videoDir)
    ...
    /* no video — fall through with null result */
  }
...
export class ReplayRecordAction
  implements ActionHandler<[url: string, opts?: ReplayOptions], Promise<ReplayResult>>
{
  async run(url: string, opts: ReplayOptions = {}): Promise<ReplayResult> {
    return recordReplayRuntime(url, opts);
  }
}
...
export async function recordReplay(
  ...
  return new ReplayRecordAction().run(url, opts);
}

export function replayEmbedHtml(result: ReplayResult): string {
