...
import { crawlPage } from "./crawl.action";
import { statSync, existsSync, readFileSync } from "fs";
import { diffDesigns } from "./diff.action.js";
import { nameFromUrl } from "@/helpers/general.helpers.js";
import { type ActionHandler } from "./action.protocol";

function fileKb(p: string): number {
  try {
...
interface VisualDiffPair {
  key: string;
  before: ScreenshotMeta | null;
  after: ScreenshotMeta | null;
}

interface VisualDiffResult {
  pairs: VisualDiffPair[];
  tokenDelta: ReturnType<typeof diffDesigns>;
  before: unknown;
  after: unknown;
}

interface ChangedColorEntry {
  token?: string;
  key?: string;
  before?: string;
  after?: string;
...
async function captureFor(url: string, options: Record<string, unknown> = {}) {
...
export class VisualDiffAction
  implements ActionHandler<[opts: VisualDiffOptions], Promise<VisualDiffResult>>
{
  async run({
    beforeUrl,
    afterUrl,
    options = {},
  }: VisualDiffOptions): Promise<VisualDiffResult> {
  const before = await captureFor(beforeUrl, options);
  const after = await captureFor(afterUrl, options);
  const tokenDelta = diffDesigns(
    before.design as Parameters<typeof diffDesigns>[0],
    after.design as Parameters<typeof diffDesigns>[1],
  );

  const shotsBefore = before.raw.componentScreenshots || {};
  const shotsAfter = after.raw.componentScreenshots || {};
  const keys = [
    ...new Set([...Object.keys(shotsBefore), ...Object.keys(shotsAfter)]),
  ];

  const pairs: VisualDiffPair[] = keys.map((k) => {
    const a = shotsBefore[k]?.path;
    const b = shotsAfter[k]?.path;
    return {
      key: k,
...
export async function visualDiff(opts: VisualDiffOptions): Promise<VisualDiffResult> {
  return new VisualDiffAction().run(opts);
}

interface VisualDiffHtmlOptions {
  beforeLabel?: string;
  afterLabel?: string;
}

export function formatVisualDiffHtml(
  result: VisualDiffResult,
  { beforeLabel, afterLabel }: VisualDiffHtmlOptions = {},
): string {
  const { pairs, tokenDelta, before, after } = result;
  const beforeUrl = (before as { meta?: { url?: string } })?.meta?.url || "before";
  const afterUrl = (after as { meta?: { url?: string } })?.meta?.url || "after";
  const bL = beforeLabel || nameFromUrl(beforeUrl);
  const aL = afterLabel || nameFromUrl(afterUrl);
  ...
  const changedColors = (
    (tokenDelta as { colors?: { changed?: ChangedColorEntry[] } }).colors?.changed ||
    []
  ).slice(0, 20);
  const tokenRows = changedColors
    .map(
      (c) =>
        `<tr><td>${c.token || c.key || ""}</td><td style="background:${c.before};">${c.before}</td><td style="background:${c.after};">${c.after}</td></tr>`,
    )
    .join("");
...
<head>
<meta charset="utf-8">
<title>Flyrank Visual DNA · visual-diff · ${bL} → ${aL}</title>
<style>
